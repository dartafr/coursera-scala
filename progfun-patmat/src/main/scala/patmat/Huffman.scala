...
object Huffman {
  ...
  def until(break: List[CodeTree] => Boolean, combiner: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = singleton(trees) match {
    case true => trees(0)
    case false => until(break, combiner)(combiner(trees))
  }
